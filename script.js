...
function renderCatalog(filter = {}) {
  ...
  const filtered = machines.filter(m => 
    ...
    (!filter.manufacturer || m.manufacturer.toLowerCase().includes(filter.manufacturer.toLowerCase())) &&
    (!filter.country || m.country.toLowerCase().includes(filter.country.toLowerCase())) &&
    (!filter.year || m.year == filter.year) &&
    (!filter.power || m.power == filter.power) &&
...
function showPopup(machine) {
  ...
  if (Array.isArray(machine.uniqueSpecs)) {
    machine.uniqueSpecs.forEach(s => specsHTML += `<li><strong>${escapeHtml(s.label)}:</strong> ${escapeHtml(s.value)}</li>`);
  }
...
if (prevBtn) prevBtn.addEventListener("click", e => { e.stopPropagation(); currentIndex = (currentIndex - 1 + currentImages.length) % currentImages.length; updatePopupImage(); });
if (nextBtn) nextBtn.addEventListener("click", e => { e.stopPropagation(); currentIndex = (currentIndex + 1) % currentImages.length; updatePopupImage(); });
